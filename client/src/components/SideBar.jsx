...
const SideBar = () => {
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 pb-[27px] border-b border-pink-200 flex justify-between items-center">
            <h2 className="text-xl font-bold text-red-600">Matches</h2>
            <button
            ...
            {isLoadingMyMatches ? (
              ...
              matches.map((match) => (
                <Link key={match._id} to={`/chat/${match._id}`}>
                  <div className="flex items-center mb-4 cursor-pointer hover:bg-red-50 p-2 rounded-lg transition-colors duration-300">
                    <img
                      src={match.image || "/avatar.png"}
                      alt="User avatar"
                      className="size-12 object-cover rounded-full mr-3 border-2 border-red-300"
                    />
      ...
      <button
        className="lg:hidden fixed top-4 left-4 p-2 bg-red-300 text-white rounded-md z-0"
        onClick={toggleSidebar}
      >
